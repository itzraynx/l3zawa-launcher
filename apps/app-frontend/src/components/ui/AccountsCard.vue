<template>
        ...
        >
                ...
                <ButtonStyled type="outlined">
                        <button :disabled="loginDisabled" @click="showOfflineModal()">
                                <UserRoundIcon />
        ...
        <AddOfflineAccountModal ref="offlineModal" @success="onOfflineSuccess" />
</template>
...
type MinecraftCredential = {
        ...
        // The auth token is present on the serialized Credentials object from the
        // Rust backend. For offline accounts it equals the sentinel "offline".
        access_token?: string
}
...
const offlineModal = ref<InstanceType<typeof AddOfflineAccountModal> | null>(null)
...
function isOfflineAccount(account: MinecraftCredential): boolean {
...
async function onOfflineSuccess(credentials: MinecraftCredential) {
        // The backend marks the new offline account as active, so we refresh to
        // pick up the updated account list and default user.
        await refreshValues()
        if (credentials?.profile?.id) {
                defaultUser.value = credentials.profile.id
        }
        emit('change')
}
